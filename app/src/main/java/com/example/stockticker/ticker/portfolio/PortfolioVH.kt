...
import com.example.stockticker.R.color.*
import com.example.stockticker.ticker.network.data.Quote
import com.example.stockticker.ticker.portfolio.drag_drop.ItemTouchHelperViewHolder

abstract class PortfolioVH(itemView: View) :
    RecyclerView.ViewHolder(itemView), ItemTouchHelperViewHolder {

    protected val positiveColor = getColor(itemView.context, positive_green)
    protected val negativeColor = getColor(itemView.context, negative_red)
    protected val neutralColor = getColor(itemView.context, text_1)
